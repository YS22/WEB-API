...
def add():
    ...
    group=Group(name=name,createTime=date.strftime('%Y-%m-%d',time.localtime(time.time())),createrId=createrid)
...
def apply():
    ...
    if groups:
        ...
        if groups not in usergroup:
            ...
            if not inspect:
                inspect=Inspect(groupid=groupId,userid=userId,time=date.time.strftime('%Y-%m-%d',time.localtime(time.time())),createrid=groups.createrId)
                db.session.add(inspect)
                db.session.commit()
                return json.dumps(creater.nickname)
            else:
                inspect.time=date.strftime('%Y-%m-%d',time.localtime(time.time()))
                db.session.add(inspect)
                db.session.commit()
                return json.dumps(creater.nickname)
        else:
